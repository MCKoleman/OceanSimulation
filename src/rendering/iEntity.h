...
public:
	...
	virtual void Draw(Shader* shader, State* state, const glm::mat4& viewProj, bool drawMats = false) = 0;
	...
	IEntity(const std::string& name = "", bool drawOver = false,
		const glm::vec3& pos = glm::vec3(0.0f), const glm::vec3& rot = glm::vec3(0.0f), const glm::vec3& scale = glm::vec3(1.0f))
		: mName(name), mDrawOver(drawOver), mPos(pos), mRot(rot), mScale(scale)
	{
		CalcBasis();
	}
